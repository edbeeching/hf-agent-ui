...
hf-agent-ui is pre-release software. Security fixes target the `main` branch until versioned releases are established.

## Threat Model

hf-agent-ui is a trusted-admin control plane for local and remote AI coding sessions. Anyone with browser access to the hub can potentially control connected agent hosts, send terminal input, and view terminal output. Do not expose the hub to untrusted users.
